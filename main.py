...
# --- Tab 2: 종목 리스트 (통합 컨트롤러 위치) ---
with tab2:
    # 시장 전환 스위치를 상단에 배치
    selected_market = st.radio("시장 선택", ["NASDAQ", "KOSPI"], horizontal=True, key="market_selector")
    
    st.markdown(f"""<div class="list-row" style="background-color: #f8f9fa; border-top: 2px solid #333; margin-top: 10px;">
        <div class="list-header">{selected_market} 종목명</div>
...
# --- Tab 3: 개별 종목 차트 (2번 탭 설정 연동) ---
with tab3:
    st.info(f"현재 **{selected_market}** 차트를 표시하고 있습니다. (시장은 '종목 리스트' 탭에서 변경 가능)")
