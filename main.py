...
# 2. 타이틀 크기 조절용 스타일 (딱 이것만 넣었습니다)
st.markdown("""
    <style>
    .small-title {
        font-size: 1.2rem !important; /* 지수 크기와 비슷하게 조절 */
        font-weight: bold;
        margin-bottom: 1rem;
    }
    </style>
...
def get_market_data():
    ...
    for name, ticker in tickers.items():
        try:
            ...
            info.append({"name": name, "val": "Error", "diff": "-"})
    return info
...
# 5. 메인 화면 상단 - 타이틀 및 지수 정보
st.markdown('<div class="small-title">📈 비서표 투자 대시보드</div>', unsafe_allow_html=True)
