...
st.markdown("""
    ...
    .list-header { font-weight: bold; flex: 1; }
    </style>
...
def get_market_data():
    ...
    tickers = {
        ...
        "NASDAQ": "^IXIC", 
        "USD-KRW": "KRW=X",
        "BRL-KRW": "BRLKRW=X", # 야후 파이낸스 표준 티커
        "GOLD": "GC=F"
    }
    info = []
...
with tab1:
    ...
    if m_info:
        # 지표 개수에 맞춰 컬럼을 생성 (5개 지표면 5개 컬럼)
        m_cols = st.columns(len(m_info))
        for i, m in enumerate(m_info):
            with m_cols[i]:
                st.markdown(f"""
                    <div class="metric-container">
                        <div style="color:#666; font-size:0.9rem;">{m['name']}</div>
                        <div class="metric-text {m['status']}">{m['val']}</div>
                    </div>
                """, unsafe_allow_html=True)
        
        st.divider()
        # 차트 영역 (상위 4개: KOSPI, NASDAQ, 환율, 금)
        c_cols = st.columns(2)
...
with tab2:
    selected_market = st.radio("표시 시장", ["NASDAQ", "KOSPI"], horizontal=True, label_visibility="collapsed", key="market_radio")
